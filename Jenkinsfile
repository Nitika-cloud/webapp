pipeline {
	...
	stage ('Deploy-To-Tomcat') {
           ...
           sshagent(['tomcat-system']) {
                sh 'scp -o StrictHostKeyChecking=no target/*.war root@35.154.252.242:/var/lib/tomcat10/webapps/webapp.war'
              }      
           }       
        }
	stage ('SAST') {
	...
	stage ('Port Scan') {
              steps {
			sh 'rm nmap* || true'
			sh 'docker run --rm -v "$(pwd)":/data uzyexe/nmap -sS -sV -oX nmap 65.0.86.79'
			sh 'cat nmap'
		    }
	    }
	stage ('DAST') {
		    	...
		    	steps {
			    sshagent(['zap']) {
				    sh 'ssh -o StrictHostKeyChecking=no ubuntu@13.243.140.198 "sudo docker run -t owasp/zap2docker-stable zap-baseline.py -t http://65.0.86.79:8080/webapp/" || true'
			    }
			}
		} 
        stage ('Nikto Scan') {
